...
export async function ensureDefaultConsentTypes(repo: Repository<ConsentType>) {
  ...
  const existing = await repo.createQueryBuilder('t').where('t.slug IN (:...slugs)', { slugs }).getMany();
  const have = new Set(existing.map(e => e.slug));
  const toCreate = required.filter(r => !have.has(r.slug)).map(r => repo.create(r));
